...
void plotMcPhiEta(int energy = 6)
{
  ...
  const int Bin_PtStart = 5; // 0.4 - 3.0 GeV/c
  const int Bin_PtStop  = 30;
  ...
  TH1F *h_CosKaon[20], *h_CosPhi[20], *h_CosKOnly[20], *h_CosKpT[20];
  float Norm_Kaon[20], Norm_Phi[20], Norm_KOnly[20], Norm_KpT[20];
  float rho_Kaon[20], rho_Phi[20], rho_KOnly[20], rho_KpT[20];
  float err_Kaon[20], err_Phi[20], err_KOnly[20], err_KpT[20];
  ...
  TGraphAsymmErrors *g_KpT = new TGraphAsymmErrors();
  TGraphAsymmErrors *g_STAR = new TGraphAsymmErrors();
  for(int i_eta = 0; i_eta < 20; ++i_eta)
  {
    ...
    h_CosEtaKpT[i_eta] = (TH2F*)File_InPut->Get(HistName.c_str());
    HistName = Form("h_CosKpT_%d",i_eta);
    // 0.4-3.0 GeV/c
    h_CosKpT[i_eta] = (TH1F*)h_CosEtaKpT[i_eta]->ProjectionY(HistName.c_str(),Bin_PtStart,Bin_PtStop);
    h_CosKpT[i_eta]->SetTitle("");
    h_CosKpT[i_eta]->SetStats(0);
    h_CosKpT[i_eta]->GetXaxis()->SetTitle("cos(#theta*)");
    h_CosKpT[i_eta]->GetXaxis()->CenterTitle();
    h_CosKpT[i_eta]->GetXaxis()->SetLabelSize(0.04);
    h_CosKpT[i_eta]->GetXaxis()->SetNdivisions(505);

    h_CosKpT[i_eta]->GetYaxis()->SetTitle("Counts");
    h_CosKpT[i_eta]->GetYaxis()->SetTitleSize(0.04);
    h_CosKpT[i_eta]->GetYaxis()->CenterTitle();
    h_CosKpT[i_eta]->GetYaxis()->SetLabelSize(0.04);
    h_CosKpT[i_eta]->GetYaxis()->SetNdivisions(505);
    h_CosKpT[i_eta]->GetYaxis()->SetRangeUser(0.9*h_CosKpT[i_eta]->GetMinimum(),1.1*h_CosKpT[i_eta]->GetMaximum());

    h_CosKpT[i_eta]->SetMarkerStyle(24);
    h_CosKpT[i_eta]->SetMarkerSize(1.1);
    h_CosKpT[i_eta]->SetMarkerColor(2);
    h_CosKpT[i_eta]->SetLineColor(2);
    TF1 *f_rhoKpT = new TF1("f_rhoKpT",SpinDensity,-1.0,1.0,2);
    f_rhoKpT->SetParameter(0,0.33);
    f_rhoKpT->SetParameter(1,100);
    h_CosKpT[i_eta]->Fit(f_rhoKpT,"NQ");
    rho_KpT[i_eta] = f_rhoKpT->GetParameter(0);
    err_KpT[i_eta] = f_rhoKpT->GetParError(0);
    Norm_KpT[i_eta] = f_rhoKpT->GetParameter(1);
    g_KpT->SetPoint(i_eta,vmsa::McEtaBin[i_eta],rho_KpT[i_eta]);
    g_KpT->SetPointError(i_eta,0.0,0.0,err_KpT[i_eta],err_KpT[i_eta]);
  }
  cout << "eta = " << vmsa::McEtaBin[9] << ", rho = " << rho_KpT[9] << endl;
  g_STAR->SetPoint(0,vmsa::mEnergyValue[energy],rho_KpT[9]);
  g_STAR->SetPointError(0,0.0,0.0,err_KpT[9],err_KpT[9]);

  TCanvas *c_EtaCutEff = new TCanvas("c_EtaCutEff","c_EtaCutEff",10,10,1600,1600);
  c_EtaCutEff->Divide(2,2);
  ...
  f_rhoKaonQA->SetParameter(0,rho_Kaon[9]);
  f_rhoKaonQA->SetParameter(1,Norm_Kaon[9]);
  ...
  string printRhoKaon = Form("#rho_{00} = %1.3f #pm %1.3f",rho_Kaon[9],err_Kaon[9]);
  plotTopLegend((char*)printRhoKaon.c_str(),0.2,0.70,0.04,1,0.0,42,1);

  c_EtaCutEff->cd(2);
  h_CosPhi[9]->Draw("pE");
  TF1 *f_rhoPhiQA = new TF1("f_rhoPhiQA",SpinDensity,-1.0,1.0,2);
  f_rhoPhiQA->SetParameter(0,rho_Phi[9]);
  f_rhoPhiQA->SetParameter(1,Norm_Phi[9]);
  ...
  plotTopLegend("|#eta| < 1.0",0.2,0.75,0.04,1,0.0,42,1);
  string printRhoPhi = Form("#rho_{00} = %1.3f #pm %1.3f",rho_Phi[9],err_Phi[9]);
  plotTopLegend((char*)printRhoPhi.c_str(),0.2,0.70,0.04,1,0.0,42,1);

  c_EtaCutEff->cd(3);
  h_CosKOnly[9]->Draw("pE");
  TF1 *f_rhoKOnlyQA = new TF1("f_rhoKOnlyQA",SpinDensity,-1.0,1.0,2);
  f_rhoKOnlyQA->SetParameter(0,rho_KOnly[9]);
  f_rhoKOnlyQA->SetParameter(1,Norm_KOnly[9]);
  f_rhoKOnlyQA->SetLineColor(2);
  f_rhoKOnlyQA->SetLineWidth(2);
  f_rhoKOnlyQA->SetLineStyle(1);
  f_rhoKOnlyQA->Draw("l same");
  plotTopLegend("cut on Kaons only",0.2,0.80,0.04,1,0.0,42,1);
  plotTopLegend("|#eta| < 1.0",0.2,0.75,0.04,1,0.0,42,1);
  string printRhoKOnly = Form("#rho_{00} = %1.3f #pm %1.3f",rho_KOnly[9],err_KOnly[9]);
  plotTopLegend((char*)printRhoKOnly.c_str(),0.2,0.70,0.04,1,0.0,42,1);

  c_EtaCutEff->cd(4);
  h_CosKpT[9]->Draw("pE");
  TF1 *f_rhoKpTQA = new TF1("f_rhoKpTQA",SpinDensity,-1.0,1.0,2);
  f_rhoKpTQA->SetParameter(0,rho_KpT[9]);
  f_rhoKpTQA->SetParameter(1,Norm_KpT[9]);
  f_rhoKpTQA->SetLineColor(2);
  f_rhoKpTQA->SetLineWidth(2);
  f_rhoKpTQA->SetLineStyle(1);
  f_rhoKpTQA->Draw("l same");
  plotTopLegend("cut on Kaons only",0.2,0.80,0.04,1,0.0,42,1);
  plotTopLegend("|#eta| < 1.0 && 0.4 < p_{T} < 3.0 GeV/c",0.2,0.75,0.04,1,0.0,42,1);
  string printRhoKpT = Form("#rho_{00} = %1.3f #pm %1.3f",rho_KpT[9],err_KpT[9]);
  plotTopLegend((char*)printRhoKpT.c_str(),0.2,0.70,0.04,1,0.0,42,1);
  ...
  g_KpT->SetMarkerStyle(20);
  g_KpT->SetMarkerColor(4);
  g_KpT->SetMarkerSize(1.4);
  g_KpT->Draw("pE same");
  ...
  leg->AddEntry(g_KpT,"cut on K and #phi-meson with STAR p_{T} cuts","p");
  leg->Draw("same");
